# from flask import Flask,request,render_template,jsonify
# from src.pipelines.prediction_pipeline import CustomData,PredictPipeline


# application=Flask(__name__)

# app=application

# @app.route('/')
# def home_page():
#     return render_template('index.html')

# @app.route('/predict',methods=['GET','POST'])

# def predict_datapoint():
#     if request.method=='GET':
#         return render_template('form.html')
    
#     else:
#         data=CustomData(
#             carat=float(request.form.get('carat')),
#             depth = float(request.form.get('depth')),
#             table = float(request.form.get('table')),
#             x = float(request.form.get('x')),
#             y = float(request.form.get('y')),
#             z = float(request.form.get('z')),
#             cut = request.form.get('cut'),
#             color= request.form.get('color'),
#             clarity = request.form.get('clarity')
#         )
#         final_new_data=data.get_data_as_dataframe()
#         predict_pipeline=PredictPipeline()
#         pred=predict_pipeline.predict(final_new_data)

#         results=round(pred[0],2)

#         return render_template('form.html',final_result=results)
    

# if __name__=="__main__":
#     app.run(host='0.0.0.0',debug=True)

import streamlit as st
from src.pipelines.prediction_pipeline import CustomData, PredictPipeline

# Function for the prediction pipeline
def predict():
    st.title("Diamond Price Prediction")

    # Input fields for the user to enter data
    carat = st.number_input("Carat", min_value=0.0, step=0.01)
    depth = st.number_input("Depth", min_value=0.0, step=0.01)
    table = st.number_input("Table", min_value=0.0, step=0.01)
    x = st.number_input("X", min_value=0.0, step=0.01)
    y = st.number_input("Y", min_value=0.0, step=0.01)
    z = st.number_input("Z", min_value=0.0, step=0.01)

    # Dropdowns for categorical values
    cut = st.selectbox("Cut", options=["Fair", "Good", "Very Good", "Ideal", "Excellent"])
    color = st.selectbox("Color", options=["J", "I", "H", "G", "F", "E", "D"])
    clarity = st.selectbox("Clarity", options=["SI1", "SI2", "VS1", "VS2", "VVS1", "VVS2", "IF"])

    # Button to trigger prediction
    if st.button("Predict"):
        # Create CustomData instance
        data = CustomData(
            carat=carat,
            depth=depth,
            table=table,
            x=x,
            y=y,
            z=z,
            cut=cut,
            color=color,
            clarity=clarity
        )
        
        # Get data as dataframe for prediction
        final_new_data = data.get_data_as_dataframe()
        
        # Predict using the pipeline
        predict_pipeline = PredictPipeline()
        pred = predict_pipeline.predict(final_new_data)

        # Display the prediction
        results = round(pred[0], 2)
        st.write(f"Predicted Diamond Price: ${results}")

if __name__ == '__main__':
    predict()
